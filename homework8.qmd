...
```{r}
rec_1 <- recipe(count ~ all_numeric_predictors(), data = bike_train) |>
  step_date(date, "dow") |> 
  update_role(count, new_role = "outcome") |>
  update_role(date, new_role = "ID") |>
  step_mutate(weekday_flag = factor(
              if_else(datedow %in% c("saturday","sunday"),0,1),
               levels = c(0,1),
...
This recipe explicitly adds interaction terms

```{r}
rec_1 <- recipe(count ~ . + (season_f:holiday_f) + (season_f:avg_temp) + (avg_temp:avg_rain), data = bike_train) |>
  step_date(date, "dow") |> 
  update_role(count, new_role = "outcome") |>
  update_role(date, new_role = "ID") |>
  step_mutate(weekday_flag = factor(
